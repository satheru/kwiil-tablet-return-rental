...
class ReturnWindow(tk.Tk):
    def __init__(self, username, on_goto_main , on_goto_login ):
        ...
        self.returnpot = set()  # 중요!!!! 여기에 담아서 한번에 넘김.
        
        
        # tkinter 윈도우 생성
        self.title("스마트 기기 반납 반납 프로그램 : 반납")
        self.state('zoomed')
        ...
        label2 = tk.Label(label2_frame, text="스마트 기기 대출 반납 프로그램 : 반납", font=("맑은 고딕", 50), bg="gray", fg="white", anchor="w")
        ...
        # 버튼들을 담을 프레임 생성 (취소 버튼과 반납 버튼을 세로로 배치)
        ...
        # 반납 버튼 (취소 버튼 아래에 배치)
        self.processing_button = tk.Button(button_frame, text="반납", font=("맑은 고딕", 30), command=self.processing, height=2, width=10)
    ...
    def add(self):
        ...
        number = exfind.barcodefind(self.username, barcode, "r")
        
        if number == False:  # 본인이 아닐 때
            msgbox.showinfo("오류", "본인이 반납한 태블릿이 아닙니다")
            self.entry.delete(0, END)  # 입력창의 텍스트를 지움
        
        else:
            if isinstance(number, int):  # 정상 상황일 때
                if not number in self.returnpot:  # 처음 선택할 때
                    self.entry.delete(0, END)  # 입력창의 텍스트를 지움
                    self.returnpot.add(number)
                    
                    # Listbox의 기존 값들 가져오기
                    current_values = list(self.listbox.get(0, END))
                    ...
                    # 새로 추가된 항목의 인덱스를 찾음
                    new_index = current_values.index(new_item)
                    
                    # Listbox 갱신
                    self.listbox.delete(0, END)  # 기존 항목 삭제
                    for value in current_values:
                        self.listbox.insert(END, value)  # 정렬된 항목 다시 추가
                    
                    # 새로 추가된 항목을 선택
                    self.listbox.select_set(new_index)
                    self.listbox.activate(new_index)
                    self.listbox.see(new_index)  # 마지막에 추가된 값이 보이도록 스크롤
    ...
    def remove(self):
        ...
        if selected_index:
            ...
            self.returnpot.discard(number)
            
    def processing(self):
        
        
        for value in sorted(self.returnpot):
            exwrite.rwrite(value,self.username)
            
        #초기화
        a = self.returnpot 
        self.returnpot = set()
        self.listbox.delete(0, END)
        msgbox.showinfo("완료", f"{a}번 태블릿을 반납했습니다")
        self.destroy()
        self.on_goto_login()
